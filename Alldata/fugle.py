...
try:
    for company in AllStockdata:
        if os.path.isfile('{}_change.csv'.format(company["stock_id"])) is False and os.path.isfile('{}_data.csv'.format(company["stock_id"])) is True:
            df_change = pd.DataFrame(
                columns=['Date', 'Open', 'High', 'Low', 'Close', 'Adj Close', 'Volume'], index=None)
            # df = pd.read_csv('{}_data.csv'.format(company["stock_id"]), usecols=[5])
            df = pd.read_csv('{}_data.csv'.format(company["stock_id"]))
            ...
            # Date,Open,High,Low,Close,Adj Close,Volume
                df_change = df_change.append({'Date': pd.to_datetime(data['date']), 'Open': data['open'], 'High': data['max'],
                                              'Low': data['min'], 'Close': data['close'], 'Adj Close': data['close'], 'Volume': data['Trading_Volume']}, ignore_index=True)

                # data = eval('{}{}{}'.format('"', row[0], '"'))
                # print(data['date'])

            df_change_reindex = df_change.set_index('Date')
            df_change_reindex.to_csv('{}_change.csv'.format(company["stock_id"]))
            print('success in {}'.format(company["stock_id"]))
        else:
            print('{}_change.csv'.format(company["stock_id"]) + ' already exist')
except:
    print('error in {}'.format(company["stock_id"]))
    print('error msg: {}'.format(sys.exc_info()))
    pass
